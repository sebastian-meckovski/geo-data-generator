...
alternate_names_df = pd.read_csv("alternate_names.txt", sep="\t", header=None, names=["geonameId", "isoLanguage", "name"],
    dtype={"geonameId": int, "isoLanguage": str, "name": str}, )
cities_df = pd.read_csv("cities.txt", sep=" ", header=None,
    names=["geonameId", "city_name", "code", "country_code"],
    dtype={"geonameId": int, "city_name": str, "code": int, "country_code": int}, )
...
def join_and_localize(language_code):
    ...
    # Merge cities_df with alternate_names_df on geonameId
    merged_df = pd.merge(cities_df, alternate_names_df[alternate_names_df["isoLanguage"] == language_code],
        left_on="geonameId", right_on="geonameId", how="left", )
    ...
    merged_df = pd.merge(merged_df, alternate_names_df[alternate_names_df["isoLanguage"] == language_code], left_on="code",
        right_on="geonameId", how="left", )
    ...
    merged_df = pd.merge(merged_df, alternate_names_df[alternate_names_df["isoLanguage"] == language_code],
        left_on="country_code", right_on="geonameId", how="left", )
    merged_df["country_name"] = merged_df["name"].fillna(merged_df["country_name"])

    # Select and rename columns
    result_df = merged_df[["geonameId", "city_name", "admin1_name", "country_name"]]
