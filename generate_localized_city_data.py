...
# define column names
cities_column_names = ["geonameId", "name", "asciiname", "alternatenames", "latitude", "longitude", "feature_class",
    "feature_code", "country_code", "cc2", "admin1_code", "admin2_code", "admin3_code", "admin4_code", "population",
    "elevation", "dem", "timezone", "modification_date", ]
alternate_names_columns = ["alternateNameId", "geonameId", "isoLanguage", "alternateName", "isPreferredName",
...
cities_dtype = {"geonameId": "int64", "name": "str", "asciiname": "str", "alternatenames": "str", "latitude": "float64",
    "longitude": "float64", "feature_class": "str", "feature_code": "str", "country_code": "str", "cc2": "str",
    "admin1_code": "str", "admin2_code": "str", "admin3_code": "str", "admin4_code": "str", "population": "int64",
    "elevation": "float64", "dem": "int64", "timezone": "str", "modification_date": "str", }

# List of feature codes to filter by
feature_codes = ["PPLA", "PPLC", "PPL", "PPLW", "PPLG", "PPLL", "PPLS", "PPLF", "PPLR", ]

# Read the data into a pandas DataFrame objects
df_cities = pd.read_csv(cities_input_file, sep="\t", header=None, names=cities_column_names, dtype=cities_dtype,
    low_memory=False, keep_default_na=False, na_values="", encoding="utf-8", )
df_alternate_names = pd.read_csv(alternate_names_file, sep="\t", header=None, names=alternate_names_columns, dtype=str,
    keep_default_na=False, na_values="", encoding="utf-8", )
admin_df = pd.read_csv(admin_file_path, sep="\t", header=None, names=admin_columns, low_memory=False,
    encoding="utf-8", )
...
filtered_df_by_population = df_cities[
    df_cities["feature_code"].isin(feature_codes) & (df_cities["population"] >= 15000)].copy()
...
df_lang[["isPreferredName", "isShortName", "isColloquial", "isHistoric"]] = df_lang[
    ["isPreferredName", "isShortName", "isColloquial", "isHistoric"]].fillna(0)
...
df_lang = df_lang[alternate_names_columns]  # Removed 'priority'

# 5. Merge allCountries with admin1CodesASCII with alternateNamesV2.
# If language not available - use original from allCountries
